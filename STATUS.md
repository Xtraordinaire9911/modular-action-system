...
| Probabilistic/Bayesian fusion | Configurable gate implemented, default unchanged | Bayesian can be evaluated as a report-only comparator, as shadow-mode evidence, or as a configurable `EpistemicArbiter(fusion_strategy="bayesian_gate")` gate that controls `allow_system1` while preserving existing fused-state selection. Initial clean 30×7 holdout ties rule-first; initial and independent-rerun live ambiguous holdouts both favor Bayesian shadow with no false-halt regression. Default production construction remains rule-first until a dedicated gate-enabled rerun is approved. | `tests/test_bayesian_fusion_comparator.py`, `tests/test_fusion_shadow_strategies.py`, `tests/test_bayesian_shadow_stability.py`, `tests/test_epistemic_runtime.py`; `artifacts/bayesian_fusion_comparator/bayesian_fusion_comparator_report.json`, `artifacts/bayesian_vs_rule_first_ablation/bayesian_vs_rule_first_ablation_report.json`, `artifacts/bayesian_shadow_stability/bayesian_shadow_stability_report.json` |
...
| Live ambiguous fusion campaign | Initial + independent rerun holdouts complete | Ambiguous profiles use `stale_offset`, `read_delay_ms`, `drop_probability`, and `source_reliability` metadata in the smart-room environment. Two 120-trial campaigns have 20/10 per-profile locked holdouts; Bayesian shadow improves balanced accuracy in both while production fusion remains unchanged. | `tests/test_live_ambiguous_fusion_campaign.py`, `tests/test_live_ambiguous_fusion_holdout.py`; `artifacts/live_ambiguous_fusion_full/live_ambiguous_fusion_summary.json`, `artifacts/live_ambiguous_fusion_rerun/live_ambiguous_fusion_summary.json`, `artifacts/live_ambiguous_fusion_holdout/live_ambiguous_fusion_holdout_report.json`, `artifacts/live_ambiguous_fusion_rerun_holdout/live_ambiguous_fusion_holdout_report.json` |
