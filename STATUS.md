...
| Probabilistic/Bayesian fusion | Experimental comparator + shadow stability complete | Bayesian can be evaluated either as a report-only comparator or through a shadow-mode strategy interface that records decisions alongside the production rule-first gate without changing runtime behavior. Initial clean 30×7 holdout ties rule-first; initial and independent-rerun live ambiguous holdouts both favor Bayesian shadow with no false-halt regression. Production fusion remains unchanged pending integration design review. | `tests/test_bayesian_fusion_comparator.py`, `tests/test_fusion_shadow_strategies.py`, `tests/test_bayesian_shadow_stability.py`; `artifacts/bayesian_fusion_comparator/bayesian_fusion_comparator_report.json`, `artifacts/bayesian_vs_rule_first_ablation/bayesian_vs_rule_first_ablation_report.json`, `artifacts/bayesian_shadow_stability/bayesian_shadow_stability_report.json` |
