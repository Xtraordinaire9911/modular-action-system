...
| Live ambiguous fusion campaign | Fine-grained 30×4 campaign complete | Ambiguous profiles use `stale_offset`, `read_delay_ms`, `drop_probability`, and `source_reliability` metadata in the smart-room environment. On the initial 120-trial live ambiguous campaign, the Bayesian comparator outperforms the fixed rule threshold, but production fusion remains unchanged pending integration design. | `tests/test_live_ambiguous_fusion_campaign.py`; `artifacts/live_ambiguous_fusion_full/live_ambiguous_fusion_summary.json` |
