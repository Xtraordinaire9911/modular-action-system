...
| Active perception | Implemented, including VLM adapter | `src/verification/active_perception.py` re-observes before continue/escalate; `src/perception/vlm_active_probe.py` turns fresh screenshot/model judgements into confidence- and provenance-bearing conflict evidence. | live `dom_wot_conflict_active_perception`; `tests/test_vlm_active_probe.py` |
...
| Recovery decision and execution | Five-family bounded recovery verified | Runtime returns non-transparent failures as `FailureContext` to the same `System2Planner`. One generic relation-driven policy recovers overlay, session continuation, optimistic compensation, visual-state recheck, and ineffective-affordance alternatives across 30 dev/holdout Chromium episodes. Autocomplete remains upstream and excluded. | `YIXIN_RUNTIME_RECOVERY_DOSSIER.md`, `tests/test_agent_runtime_replan_contract.py`, `tests/test_live_browser_claims.py`; `artifacts/generalized_browser_recovery/` |
...
| Open-web failure coverage | Six-family detection; five-family bounded recovery; real open web future | Six local families retain failure detection. Recovery excludes autocomplete and covers the other five through one generic capability contract: 30/30 Chromium dev/holdout episodes verified. This is controlled local-browser evidence, not a real open-web claim. | `YIXIN_RUNTIME_RECOVERY_DOSSIER.md`, `tests/test_open_web_randomized_holdout.py`, `tests/test_live_browser_claims.py`; `artifacts/generalized_browser_recovery/` |
