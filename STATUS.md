...
| Active perception | Runtime interface implemented; VLM implementation external | `src/verification/active_perception.py` accepts an injected perception probe and re-observes before continue/escalate. Runtime contains no VLM invocation policy. | live `dom_wot_conflict_active_perception`; VLM integration/evidence belongs to the perception owner |
