...
| Open-web failure coverage | Controlled mock suite complete; real open-web still future | Open-web failure modes are explicitly separated into mechanism-ready, controlled/mock evidence, and real open-web evidence levels. Current report covers 8 failure classes, adds 6 oracle-labeled local mock fixtures for overlay obstruction, session expiry, async validation mutation, DOM/visual disagreement, optimistic UI/backend mismatch, and visible-but-ineffective affordances, and still records zero real open-web evidence. | `tests/test_open_web_failure_coverage.py`, `tests/test_open_web_mock_failure_suite.py`; `artifacts/open_web_failure_coverage/open_web_failure_coverage_report.json`, `artifacts/open_web_mock_failure_suite/open_web_mock_failure_suite_report.json` |
