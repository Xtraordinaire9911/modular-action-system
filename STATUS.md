...
| Live ambiguous fusion campaign | Scaffold implemented, 1×4 smoke complete | Ambiguous profiles are mapped onto the current smart-room fault API and can be planned/replayed with unique episode ids and seeds. Because the environment still lacks fine-grained weak-stale/reliability/missing-probability controls, this is a live smoke bridge rather than a full ambiguous benchmark. | `tests/test_live_ambiguous_fusion_campaign.py`; `artifacts/live_ambiguous_fusion_smoke/live_ambiguous_fusion_summary.json` |
