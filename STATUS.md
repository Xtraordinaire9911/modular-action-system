...
| Recovery decision and execution | Bounded implementation; broader evidence open | Runtime owns validation, execution, fresh observation, verification, budgets, transparent retry/reroute/rollback, and escalation. Non-transparent failures become a sanitized `FailureContext` and return to the same `System2Planner`. Overlay repair is verified; four in-scope families remain external capability dependencies. Autocomplete is an upstream environment/Agent concern and does not gate Runtime recovery closure. | `YIXIN_RUNTIME_RECOVERY_DOSSIER.md`, `tests/test_runtime_episode_recovery.py`, `tests/test_precondition_repair.py`, `tests/test_live_browser_claims.py` |
