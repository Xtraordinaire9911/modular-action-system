...
| Recovery decision and execution | Bounded implementation; cross-family closure open | Runtime owns validation, execution, fresh observation, verification, budgets, transparent retry/reroute/rollback, and escalation. Non-transparent failures become a sanitized `FailureContext` and return to the same `System2Planner`; Runtime no longer calls a second repair planner. Overlay repair is verified; five families currently have typed fail-closed outcomes because required environment/evidence capabilities are absent. | `YIXIN_RUNTIME_RECOVERY_DOSSIER.md`, `tests/test_runtime_episode_recovery.py`, `tests/test_precondition_repair.py`, `tests/test_live_browser_claims.py` |
...
| Visual observation | VLM adapter implemented; external-provider evidence absent | The intent episode captures PNG bytes, sends them to a configured vision client, records confidence/provenance, and fuses accepted evidence. CI uses a fake client; no checked-in run proves an external model provider was configured. | `src/perception/vlm_observer.py`, `scripts/run_intent_episode.py`, `tests/test_vlm_observer.py`, `tests/test_live_browser_claims.py` |
...
| Open-web failure coverage | Six-family detection complete; one-family recovery; real open-web future | Six oracle-labeled local families have seeded dev/locked-holdout variants, Chromium execution, and fresh failure verification. The six successful recovery episodes are all overlay-obstruction variants (3 dev + 3 holdout), not six different failure mechanisms. Session, autocomplete, optimistic rollback, DOM/visual disagreement, and ineffective-affordance fixtures currently lack a reachable autonomous repair or sufficient evidence and therefore fail closed. | `YIXIN_RUNTIME_RECOVERY_DOSSIER.md`, `tests/test_open_web_failure_coverage.py`, `tests/test_open_web_randomized_holdout.py`, `tests/test_precondition_repair.py`, `tests/test_live_browser_claims.py` |
