...
| Probabilistic/Bayesian fusion | Experimental comparator + shadow interface implemented | Bayesian can be evaluated either as a report-only comparator or through a shadow-mode strategy interface that records decisions alongside the production rule-first gate without changing runtime behavior. Initial clean 30×7 holdout ties rule-first; live ambiguous 20/10 holdout favors Bayesian shadow but still requires independent rerun/review before production promotion. | `tests/test_bayesian_fusion_comparator.py`, `tests/test_fusion_shadow_strategies.py`; `artifacts/bayesian_fusion_comparator/bayesian_fusion_comparator_report.json`, `artifacts/bayesian_vs_rule_first_ablation/bayesian_vs_rule_first_ablation_report.json` |
...
| Live ambiguous fusion campaign | Fine-grained 30×4 campaign + locked holdout complete | Ambiguous profiles use `stale_offset`, `read_delay_ms`, `drop_probability`, and `source_reliability` metadata in the smart-room environment. The initial 120-trial campaign is split into a 20/10 per-profile locked holdout; Bayesian shadow improves balanced accuracy on the holdout while production fusion remains unchanged. | `tests/test_live_ambiguous_fusion_campaign.py`, `tests/test_live_ambiguous_fusion_holdout.py`; `artifacts/live_ambiguous_fusion_full/live_ambiguous_fusion_summary.json`, `artifacts/live_ambiguous_fusion_holdout/live_ambiguous_fusion_holdout_report.json` |
