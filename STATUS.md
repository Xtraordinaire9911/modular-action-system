...
| Probabilistic/Bayesian fusion | Configurable gate + gate-enabled live eval complete | Bayesian can be evaluated as a report-only comparator, as shadow-mode evidence, or as a configurable `EpistemicArbiter(fusion_strategy="bayesian_gate")` gate that controls `allow_system1` while preserving existing fused-state selection. Initial clean 30×7 holdout ties rule-first; initial and independent-rerun live ambiguous holdouts favor Bayesian shadow; a 120-trial gate-enabled live ambiguous campaign reaches balanced accuracy 1.0 with no misses or false halts. Default production construction remains rule-first pending final owner review. | `tests/test_bayesian_fusion_comparator.py`, `tests/test_fusion_shadow_strategies.py`, `tests/test_bayesian_shadow_stability.py`, `tests/test_epistemic_runtime.py`; `artifacts/bayesian_fusion_comparator/bayesian_fusion_comparator_report.json`, `artifacts/bayesian_vs_rule_first_ablation/bayesian_vs_rule_first_ablation_report.json`, `artifacts/bayesian_shadow_stability/bayesian_shadow_stability_report.json`, `artifacts/live_ambiguous_fusion_bayesian_gate_full/live_ambiguous_fusion_summary.json` |
