...
| Live ambiguous fusion campaign | Fine-grained fault API implemented, 1×4 smoke complete | Ambiguous profiles now use `stale_offset`, `read_delay_ms`, `drop_probability`, and `source_reliability` metadata in the smart-room environment. Production fusion is still unchanged; the live ambiguous benchmark is ready for repeated runs. | `tests/test_live_ambiguous_fusion_campaign.py`; `artifacts/live_ambiguous_fusion_fine_smoke/live_ambiguous_fusion_summary.json` |
