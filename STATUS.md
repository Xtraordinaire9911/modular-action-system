...
| Repeated fusion/recovery campaign | Scaffold implemented, full campaign pending | The campaign runner creates deterministic 7-condition × N repetition plans, records unique episode ids/seeds, reset evidence, independent oracle labels, replay config, and condition-level precision/recall/false-halt/miss/latency summaries. The 30×7 run is a long-running empirical campaign, not yet a publication-grade result. | `tests/test_live_fusion_campaign.py`; `python -m src.pipeline --fusion-campaign-dry-run`; 1×7 live smoke |
