...
# cursor to the target with a fluorescent (red-theme) glowing trail, pulse an
# outline on the element, and show a floating caption. Returns whether the
# selector resolved (callers skip safely on false). The cursor persists across
# steps so it visibly travels between targets on the same page; on a freshly
# loaded page it simply appears at the target (no misleading trail).
_FLASH_JS = """
(a)=>{
  ...
  if(!document.getElementById('__cua_style')){
    ...
    st.textContent='@keyframes __cua_fade{from{opacity:.9}to{opacity:0}}'
      ...
      +'background:radial-gradient(circle,#ff5b5b,#e11);box-shadow:0 0 8px 2px rgba(255,70,70,.85);'
  ...
  if(!cur){
    ...
    cur.style.cssText='position:fixed;z-index:100000;pointer-events:none;'
      +'transition:left .5s ease,top .5s ease;filter:drop-shadow(0 0 6px rgba(255,70,70,.9));'
      +'left:'+tx+'px;top:'+ty+'px';
    cur.innerHTML='<svg width="26" height="26" viewBox="0 0 24 24"><path d="M4 2 L4 20 L9 15 '
      +'L12.5 22 L15 21 L11.5 14 L18 14 Z" fill="#e11" stroke="#fff" stroke-width="1.3" '
  ...
  el.style.outline='3px solid #e11'; el.style.boxShadow='0 0 0 6px rgba(225,17,17,.25)'; el.classList.add('__cua_hl');
