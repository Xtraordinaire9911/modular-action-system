...
### 14.5.11 Open-web mock runtime envelope 记录

- **时间**：2026-08-06
- **代码入口**：
  - `evaluation/open_web_mock_runtime_runner.py`
  - CLI: `python -m src.pipeline --open-web-mock-runtime-suite --output-dir artifacts/open_web_mock_runtime_suite --seed-start 9000`
- **已生成 artifact**：
  - `artifacts/open_web_mock_runtime_suite/open_web_mock_runtime_episode_report.json`
  - `artifacts/open_web_mock_runtime_suite/transition_ledger.jsonl`
  - `artifacts/open_web_mock_runtime_suite/failure_ledger.jsonl`
- **runtime path**：
  - `RuntimeEpisodeRunner.run_skill_episode`
  - in-memory mock DOM executor reports `ExecutionResult.success=true`
  - fresh oracle re-observation reports `oracle.expected_effect_satisfied=false`
  - postcondition: `oracle.expected_effect_satisfied == true`
  - policy: `max_steps=1`, `max_retry_attempts=0`
- **结果**：
  - case count = 6
  - runtime episode count = 6
  - executor success count = 6
