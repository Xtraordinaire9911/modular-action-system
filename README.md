...
| A model actually running | **Wired, one real round trip attempted** | Both paths reach a real endpoint; whether a model *answers* depends on a configured key. The vision path sends a real PNG of the region the goal names to an OpenAI-compatible vision model and feeds the answer into the runtime's fusion as a `source="visual"` assertion carrying the model's own confidence, so agreement verifies the goal twice and disagreement raises a conflict. One real call has been made and was refused at the vendor's billing gate (`credit balance is too low`) before any inference ran, which exercised the failure path rather than the model. With no key configured every decision is `rule_fallback` / `heuristic` and says so. Setup, provider choice and measured per-call cost: `docs_setup/VLM_SETUP.md`. |
