...
| A model actually running | **Running, and measured** | Both layers run against a real endpoint (`qwen-plus`, `qwen-vl-plus`). Not just wired: `scripts/eval_model_value.py` asks whether they earn their place. **Intent** — on five requests phrased to avoid the fallback's keywords the rules score 0/5 and the model 4/5, with no regression on the two the rules already handled and both correctly refusing two out-of-scope requests. **Vision** — against `invisible_confirmation`, a fault that leaves the confirmation in the DOM and paints over the region, the model catches the false success the DOM confirms: detection 100% (n=5), false alarm 0% (n=10), accuracy 100%. Evidence in `artifacts/model_value/`. |
